...
setMinute()
{
  ...
  getExceptionrangeDetails() {
    this.appName=this.insightsApplicationName.replace(/[_-]/g, "")+this.insightsPipelineName.replace(/[_-]/g, "");
   if(this.insightsApplicationName==='' && this.insightsPipelineName===''){
	 return;
 }
          ...
          if (res['status'] === 200) {
            ...
            for(let exceptionObj of this.anamolyException)
            {
              ...
              for(let val of this.checkinDetails){
                ...
                if(exceptionObj[1].toLowerCase() === val.toLowerCase() && pref===0){
